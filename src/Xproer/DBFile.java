...
public class DBFile {
	...
	public boolean GetFileInfByFid(int f_id,xdb_files inf)
	{
		...
		sb.append(",f_time");
		sb.append(",f_deleted");
		sb.append(" from cloud2_files where f_id=? limit 0,1");
	...
	public boolean exist_file(String md5,/*out*/xdb_files fileSvr)
	{
		...
		sb.append(",f_time");
		sb.append(",f_deleted");
		sb.append(" from cloud2_files where f_md5=? order by f_lenSvr DESC limit 0,1");
	...
	public int Add(xdb_files model)
	{
		...
		String sql = "select f_id from cloud2_files order by f_id desc limit 0,1";		
	...
	static public int Add(FileInf inf)
	{
		...
		//int f_id = db.ExecuteScalar("select f_id from cloud2_files order by f_id desc limit 0,1");
	...
	{
		DbHelper db = new DbHelper();
		db.ExecuteNonQuery("delete from cloud2_files;");
		db.ExecuteNonQuery("delete from cloud2_folders;");
	}
	...
	{
		DbHelper db = new DbHelper();
		PreparedStatement cmd = db.GetCommand("update cloud2_files set f_perSvr='100%' ,f_complete=1 where f_id=?;");
		try {
			cmd.setInt(1, f_id);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	{
		//String sql = "update cloud2_files set f_pos=?,f_lenSvr=?,f_perSvr=? where f_uid=? and f_id=?";
	...
	{
		String sql = "update cloud2_files set f_lenSvr=f_lenLoc,f_perSvr='100%',f_complete=1 where f_md5=?";
		DbHelper db = new DbHelper();
	...
	public boolean HasCompleteFile(String md5)
	{
		...
		String sql = "select f_id from cloud2_files where f_complete=1 and f_md5=?";
		DbHelper db = new DbHelper();
		PreparedStatement cmd = db.GetCommand(sql);
	...
	{
		String sql = "update cloud2_files set f_deleted=1 where f_uid=? and f_id=?";
		DbHelper db = new DbHelper();
		PreparedStatement cmd = db.GetCommand(sql);
	...
	static public void GetUnCompletes(int fidRoot,ArrayList<FileInf> files)
	{
		...
		sql.append(" from cloud2_files where f_pidRoot=? and f_complete=False;");
