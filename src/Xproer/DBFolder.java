...
public class DBFolder {
	...
	static public int Add(FolderInf inf)
	{
		...
		String sql = "select fd_id from cloud2_folders order by fd_id desc limit 0,1";		
	...
	{
		String sql = "update cloud2_folders set fd_complete=1 where fd_id=? and fd_uid=?;";
		DbHelper db = new DbHelper();
	...
	{
		String sql = "update cloud2_folders set fd_delete=1 where fd_id=? and fd_uid=?;";
		DbHelper db = new DbHelper();
		PreparedStatement cmd = db.GetCommand(sql);
		try 
		{
	...
	static public String GetFilesUnComplete(int fid,FolderInf root)
	{
		...
		sb.append(" from cloud2_folders where fd_id=?;");
	...
	{
		String sql = "select sum(f_lenSvr) as lenPosted from (select distinct f_md5,f_lenSvr from cloud2_files where f_pidRoot=? and LENGTH(f_md5) > 0) a";
		DbHelper db = new DbHelper();
		PreparedStatement cmd = db.GetCommand(sql);
		try {
    ...
    {
        String sql = "update cloud2_folders set fd_filesComplete=fd_filesComplete+1 where fd_id=?";
        DbHelper db = new DbHelper();
        PreparedStatement cmd = db.GetCommand(sql);
		try {
